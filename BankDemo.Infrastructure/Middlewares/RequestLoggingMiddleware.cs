...
public class RequestLoggingMiddleware : IMiddleware
{
    public async Task InvokeAsync(HttpContext httpContext, RequestDelegate next)
    {
        ...
        if (httpContext?.Request.Path.ToString().Contains("tokens", StringComparison.OrdinalIgnoreCase) ?? false)
        {
        ...
        Log.ForContext("RequestHeaders",
                httpContext.Request.Headers.Where(x => x.Value.Contains("Bearer") is false)
